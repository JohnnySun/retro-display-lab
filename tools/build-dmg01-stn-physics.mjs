...
// libm differences between arm64 and x86_64 can move the last reported digit,
// especially after transition progress divides two nearly equal optical states.
// Keep the executable Shader include byte-exact below, while accepting only
// last-digit numeric drift in the human-readable JSON report.
const generatedReportTolerance = 1e-7;

function generatedReportDifference(expected, actual, location = "$") {
  if (typeof expected === "number" && typeof actual === "number") {
    const scale = Math.max(1, Math.abs(expected), Math.abs(actual));
    if (Math.abs(expected - actual) <= generatedReportTolerance * scale) return null;
    return `${location}: expected ${expected}, found ${actual}`;
  }
  if (Array.isArray(expected) || Array.isArray(actual)) {
    ...
    if (expected.length !== actual.length) {
      return `${location}: expected ${expected.length} entries, found ${actual.length}`;
    }
    for (let index = 0; index < expected.length; index += 1) {
      const difference = generatedReportDifference(
        expected[index], actual[index], `${location}[${index}]`,
      );
  ...
  if ((expected && typeof expected === "object")
    ...
    if (!expected || !actual || typeof expected !== "object" || typeof actual !== "object") {
      return `${location}: object shape changed`;
    }
    const expectedKeys = Object.keys(expected);
    const actualKeys = Object.keys(actual);
    if (expectedKeys.length !== actualKeys.length
        || expectedKeys.some((key, index) => key !== actualKeys[index])) {
      return `${location}: object keys or order changed`;
    }
    for (const key of expectedKeys) {
      const difference = generatedReportDifference(
        expected[key], actual[key], `${location}.${key}`,
      );
      if (difference) return difference;
    }
    return null;
...
if (checkOnly) {
  if (!fs.existsSync(outputPath)) {
    fail("DMG WS2 physical report is missing; run node tools/build-dmg01-stn-physics.mjs");
  }
  const storedReportText = fs.readFileSync(outputPath, "utf8");
  if (storedReportText !== reportText) {
    let difference;
    try {
      difference = generatedReportDifference(
        JSON.parse(reportText), JSON.parse(storedReportText),
      );
    } catch (error) {
      difference = `stored report is not valid JSON: ${error.message}`;
    }
    if (difference) {
      fail(`DMG WS2 physical report is stale (${difference}); run node tools/build-dmg01-stn-physics.mjs`);
    }
  }
