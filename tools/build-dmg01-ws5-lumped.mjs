...
import {
  applyCrosstalkToneGuard,
  CROSSTALK_MAX_TONE_EXCURSION,
...
for (const ensemble of evidence.ensembles) {
  ...
  for (const id of patternIds) {
    ...
    if (id === "mixed-tone-mino") {
      const guardedByShade = [[], [], [], []];
      for (let index = 0; index < pattern.shades.length; index += 1) {
        const shade = pattern.shades[index];
        guardedByShade[shade].push(applyCrosstalkToneGuard(
          shade,
          lumped.driveCoordinates[index],
          lumpedUniform.get(shade).driveCoordinates[index],
        ));
      }
      const shade2Maximum = Math.max(...guardedByShade[2]);
      const shade3Minimum = Math.min(...guardedByShade[3]);
      mixedToneOrder = {
        maximumExcursionShade: CROSSTALK_MAX_TONE_EXCURSION,
        shade2Maximum: clean(shade2Maximum),
        shade3Minimum: clean(shade3Minimum),
        minimumSeparationShade: clean(shade3Minimum - shade2Maximum),
      };
    }
  }
  ensembles.push({
...
const checks = {
  ...
  allTenPatternsCompared: ensembles.every((item) => item.patterns.length === 10),
  ...
  mixedToneOrderPreserved: nominal.mixedToneOrder.minimumSeparationShade >= 0.5,
};
const report = {
  schemaVersion: 3,
  reportId: "nintendo-dmg-01-ws5-crosstalk-lumped-v2",
  classification: "structure-preserving common-electrode-mode reduction with a discrete-tone safety boundary; no image-pattern fit",
...
const include = `// Generated by tools/build-dmg01-ws5-lumped.mjs. Do not hand-edit.\n`
  ...
  + `const float DMG_CROSSTALK_MAX_TONE_EXCURSION = ${CROSSTALK_MAX_TONE_EXCURSION.toFixed(12)};\n`
